...
RSpec.describe ActiveStorage::Service::NftService do
  let(:api_key) { ENV["NFT_STORAGE_API_KEY"] }
  let(:gateway_endpoint) { 'https://gateway.pinata.cloud' } 
  let(:key) { 'some-key' }
  let(:file) { double }
  let(:filename) { 'text.txt' }
  let(:checksum) { 'ushagte' }
  
  let(:subject) { ActiveStorage::Service::NftService.new(api_key: api_key, gateway_endpoint: gateway_endpoint) }
